...
export function LatencyOverTime({
  ...
  // Approximate bucket width from adjacent buckets so we can tag nearby
  // events to the hover position even though this chart doesn't take
  // `bucketMs` as an explicit prop.
  const bucketMs =
    sanitized.length >= 2 ? sanitized[1].bucket - sanitized[0].bucket : 60_000;
  const firstBucket = sanitized[0]?.bucket ?? 0;
  const lastBucket = sanitized[sanitized.length - 1]?.bucket ?? 0;
    ...
    <div className="card p-3 sm:p-4">
      ...
      <div className="relative">
        ...
        <div className="absolute right-1 top-1 bottom-4 flex flex-col justify-between pointer-events-none text-[9px] font-mono text-text-tertiary tabular-nums">
          <span>{niceMax >= 1000 ? `${(niceMax / 1000).toFixed(1)}s` : `${Math.round(niceMax)}ms`}</span>
          <span>{niceMax >= 1000 ? `${(niceMax / 2000).toFixed(1)}s` : `${Math.round(niceMax / 2)}ms`}</span>
          <span>0</span>
        </div>
        {hover && hoverX !== null && (
          <div
