...
export function RpsOverTime({
  ...
  // Hooks must run unconditionally — every render, same order. Derive event
  // markers from the raw series (bucket timestamps survive the per-second
  // transform below) BEFORE the empty-state early return. Putting this
  // useMemo after the `if (!hasData)` return makes the hook count differ
  // between the "no data" and "has data" renders → React error #310.
  ...
  const hoverX =
    hoverIndex !== null ? pad + (hoverIndex / Math.max(data.length - 1, 1)) * (W - pad * 2) : null;

  // firstBucket/lastBucket/rangeEvents are computed above the early return
  // (hook-order safety). hoveredEvents + tsToX are plain derivations, fine here.
        ...
        {hover && hoverX !== null && (
          ...
          >
            ...
            <p className="flex items-center justify-between gap-2">
              <span className="text-text-tertiary uppercase tracking-wider text-[9px]">Total</span>
