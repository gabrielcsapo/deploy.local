...
export function LiveStatusStrip({ name }: { name: string }) {
  ...
  const cpuTone =
    health.cpu == null ? 'default' : health.cpu >= 90 ? 'danger' : health.cpu >= 70 ? 'warning' : 'success';
  const memTone =
    health.memPct == null
      ? 'default'
      : health.memPct >= 90
        ...
        : health.memPct >= 70
          ...
          : 'success';
  const errTone =
    !health.errPct ? 'default' : health.errPct >= 5 ? 'danger' : health.errPct >= 1 ? 'warning' : 'success';
  const p95Tone =
    health.p95Ms == null
      ? 'default'
      : health.p95Ms >= 5000
...
function Pill({
  ...
  return (
    <div
      className={`shrink-0 snap-start inline-flex flex-col gap-1 px-3 py-2 rounded-lg border ${toneClass[tone]} min-w-[120px] transition-colors`}
    >
      ...
      <div className="flex items-baseline justify-between gap-2">
        <span
          key={flashKey}
          className="text-sm font-mono font-semibold text-text tabular-nums"
        >
          {value}
        </span>
